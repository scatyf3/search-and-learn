...
def best_of_n_with_draft(x, config: Config, llm: LLM, prm: PRM):
    ...
    # 1. 加载 draft_llm
    logging.info("Loading draft LLM...")
...
def best_of_n_with_draft_and_verify(x, config: Config, llm: LLM, prm: PRM):
    """
    draft_llm生成初步答案，主llm对draft结果做重写/补充（verify），再由PRM打分。
    config.draft_model_path: draft模型路径（可选）
    """
    import time
    from vllm import LLM, SamplingParams
    ...
    # 4. 主llm对draft结果做重写/补充（verify）
    verify_prompts = []
    for draft_list in completions:
        # 每个draft答案都让主llm重写/补充
        for draft_ans in draft_list:
            verify_prompts.append([
                {"role": "system", "content": "Please verify, rewrite, or supplement the following answer to make it more complete and correct."},
                {"role": "user", "content": draft_ans}
            ])
    verify_tokenizer = llm.get_tokenizer()
    verify_templated = verify_tokenizer.apply_chat_template(
        verify_prompts, tokenize=False, add_generation_prompt=True
    )
    ...
    verify_responses = llm.generate(
        verify_templated,
        sampling_params=verify_sampling_params,
        use_tqdm=False,
    )
    t_verify_end = time.time()
    verify_gen_time = t_verify_end - t_verify_start

    # 5. 还原verify结果结构
    verify_completions = [[] for _ in range(len(x["problem"]))]
    idx = 0
    for i in range(len(verify_completions)):
        verify_completions[i] = [
            verify_responses[idx + j].outputs[0].text
    ...
    # 6. PRM验证
    t_prm_start = time.time()
    scores = prm.score(x["problem"], verify_completions)
    t_prm_end = time.time()
    prm_score_time = t_prm_end - t_prm_start
    ...
    pred = [completion[np.argmax(s)] for completion, s in zip(verify_completions, agg_scores)]

    x["draft_completions"] = completions
    x["verify_completions"] = verify_completions
    x["scores"] = scores
    x["pred"] = pred
    x["completion_tokens"] = completion_tokens
    x["draft_gen_time"] = [draft_gen_time] * len(x["problem"])
    x["verify_gen_time"] = [verify_gen_time] * len(x["problem"])
    x["prm_score_time"] = [prm_score_time] * len(x["problem"])
    x["timing_n"] = [config.n] * len(x["problem"])
    x["timing_batch_size"] = [config.search_batch_size] * len(x["problem"])
    x["timing_timestamp"] = [datetime.now().strftime("%Y%m%d_%H%M%S")] * len(x["problem"])
